...
@section('content')
    ...
    @isset($nachricht)
        ...
        <h1 style="color: greenyellow"> {{$nachricht}} </h1>

    @endisset


    {!! Form::open(['action' => ['UserController@export_pruefer' , $discipline->id], 'method' => 'GET']) !!}
    <button type="submit" class="btn btn-secondary mb-2"><img class="img-fluid" src="/bilder/download.svg"> Ergebnisse ausdrucken</button>
    {!! Form::close() !!}
    ...
    <p>Um diese Disziplin zu bestehen, muss man ein Ergebnis von mindestens
    ...
    <small> Bitte beachten Sie: das Eintragen eines Fehlers in der Tabelle unten führt automatisch zum Nicht-Bestehen der Prüfung. Nutzen Sie daher ggf. die Kommentarspalte</small>
    ...
    <table class="table table-striped table-responsive-sm table-responsive-lg table-responsive-md table-responsive-xl" id="myTable">
        ...
        @foreach($prueflinge as $p)
            @if(($discipline->numberOfTry - \App\Result::where('pers_daten_pruefling_id', $p->id)->where('discipline_id', $discipline->id)->where('enteredIn',1)->count()<=0 and
                 strtotime('+1 day',strtotime(\App\Test::all()->first()->dateFirstTest)) > time()) or
                  ($discipline->numberOfTry - \App\Result::where('pers_daten_pruefling_id', $p->id)->where('discipline_id', $discipline->id)->where('enteredIn',2)->count()<=0 and
                  strtotime('+1 day',strtotime(\App\Test::all()->first()->dateFirstTest)) < time()))
                <tr>
                    <td width="15%">
                        ...
                        <small class="form-text text-muted">Versuche übrig:
                            @if(strtotime('+1 day',strtotime(\App\Test::all()->first()->dateFirstTest)) < time())
                                {{$discipline->numberOfTry - \App\Result::where('pers_daten_pruefling_id', $p->id)->where('discipline_id', $discipline->id)->where('enteredIn',2)->count()}}
                            @else
                                {{$discipline->numberOfTry - \App\Result::where('pers_daten_pruefling_id', $p->id)->where('discipline_id', $discipline->id)->where('enteredIn',1)->count()}}
                            @endif
                        </small>
                    </td>
                    <td>@if($p->passbild)
                    ...
                    <td>
                        ...
                        @if(strtotime('+1 day',strtotime(\App\Test::all()->first()->dateFirstTest)) < time())
                            <ol>
                                @foreach(\App\Result::where('pers_daten_pruefling_id', $p->id)->where('discipline_id', $discipline->id)->where('enteredIn',2)->get() as $pResults)
                                    <li>
                        ...
                        @else
                            <ol>
                                @foreach(\App\Result::where('pers_daten_pruefling_id', $p->id)->where('discipline_id', $discipline->id)->where('enteredIn',1)->get() as $pResults)
                                    <li>
                                        @if($pResults->error_code_id!=null)
                                            {{$pResults->errorCode->description}}
                                        @else
            ...
            @else
                {!! Form::open(['action' => ['ResultController@store', $p->id, $discipline->id, 5], 'method' => 'POST' ]) !!}
                <tr>
                    <td width="15%">
                        {{$p->nachname}}, {{$p->vorname}}<br>
                        <small class="form-text text-muted">Versuche übrig:
                    ...
                    <td class="pt-lg-3-half">
                        @if($discipline->measureDataType == 1)
                            <input type="time" step="0.01" name="timeResult" class="form-control" placeholder="00:00,00">
