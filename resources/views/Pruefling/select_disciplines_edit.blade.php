...
@section('content')
    <div class="d-flex justify-content-between pt-3 pb-2 mb-3 border-bottom">
        ...
        <a href="{{ url('/home') }}" class="btn btn-primary pull-right">zurück</a>
                        ...
                        @endif
                        <p>Um Ihre Auswahl zu ändern, wählen Sie bitte die Disziplinen, in denen Sie geprüft werden wollen und klicken Sie anschließend auf speichern.</p>
                                    ...
                                    @foreach($disciplines as $d)
                                        @if(($d->sport_id == $sport->id) && ($d->selectable == 0))
                                            <ul style="list-style-type:none;">
                                               <li> <img src="/icons/check-box.svg" sizes="any" alt="">  {{$d->name}}</li>
                                            </ul>
                                        @endif
                                    @endforeach
                                </ul>
